...
- `quickshell/modules/dynamicGlacier/BatteryPanel.qml`: battery health, cycles, capacity, voltage, UPower threshold control, and system power profiles.
...
- system power profiles through `org.freedesktop.UPower.PowerProfiles` when `power-profiles-daemon` is available
